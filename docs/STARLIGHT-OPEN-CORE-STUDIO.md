...
The structural reference contracts cover tool admission, workflow transitions, signed approvals, and asset readiness. `scripts/studio_policy.py` is explicitly a non-production, process-local example. `AdmissionExecutor` freezes policy and authorization verification during trusted bootstrap so operation callers cannot inject a verifier per request; `WorkflowGate` similarly receives its verifier configuration at bootstrap, and `ApprovalVerifier` exposes no signing method. The tests cover revocation and exact distribution/tool admission, path and egress containment, schema-equivalent runtime types and uniqueness, hash-bound evidence, trusted maker/checker separation, artifact/run-bound HMAC approval verification, idempotency, and atomic budget `HOLD`. This Python module is **not itself a security boundary**: the deployer must isolate bootstrap construction and signing keys from untrusted request/agent code. It does **not** provide an OS sandbox, authenticated policy distribution, persistent workflow/budget ledger, live engine adapters, or recovery orchestration.
