...
from studio_policy import (
    AdmissionExecutor, ApprovalVerifier, BudgetLedger, PolicyViolation, WorkflowGate,
    canonical_hash, validate_3d_asset,
)
...
class StudioPolicyTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.executor = AdmissionExecutor(self.policy, self.trusted_auth)
    ...
    def test_allowed_call_emits_hash_bound_receipt(self) -> None:
        result = self.executor.admit(self.call, NOW)
    ...
    def test_admission_fails_for_revocation_distribution_and_absent_trusted_auth(self) -> None:
        ...
        with self.assertRaises(PolicyViolation): AdmissionExecutor(revoked, self.trusted_auth).admit(self.call, NOW)
        wrong_dist = dict(self.call, distribution_sha256=HASH)
        with self.assertRaises(PolicyViolation): self.executor.admit(wrong_dist, NOW)
        with self.assertRaises(PolicyViolation): AdmissionExecutor(self.policy, None).admit(self.call, NOW)
        with self.assertRaises(TypeError): self.executor.admit(self.call, NOW, lambda *_: True)
    ...
    @staticmethod
    def sign_approval(secret: bytes, **payload):
        body = dict(payload)
        body["signature"] = hmac.new(secret, json.dumps(body, sort_keys=True, separators=(",", ":")).encode(), hashlib.sha256).hexdigest()
        return body

    def test_release_requires_current_signed_artifact_bound_approval(self) -> None:
        secret = b"test-secret"
        verifier = ApprovalVerifier({"human-1": secret})
        ...
        approval = self.sign_approval(secret, approver_id="human-1", run_id="run-1", artifact_hash=HASH, actor="maker", issued_at=90, expires_at=110, nonce="nonce-1")
        signed = dict(release, approval_receipt=approval)
        self.assertEqual(WorkflowGate({"checker"}, verifier).advance(signed, NOW)["decision"], "ALLOW")
        forged = dict(approval, artifact_hash=DIST)
        with self.assertRaises(PolicyViolation): WorkflowGate({"checker"}, verifier).advance(dict(release, approval_receipt=forged), NOW)
        expired = self.sign_approval(secret, approver_id="human-1", run_id="run-1", artifact_hash=HASH, actor="maker", issued_at=80, expires_at=99, nonce="nonce-2")
        with self.assertRaises(PolicyViolation): WorkflowGate({"checker"}, verifier).advance(dict(release, approval_receipt=expired), NOW)
        empty_nonce = self.sign_approval(secret, approver_id="human-1", run_id="run-1", artifact_hash=HASH, actor="maker", issued_at=90, expires_at=110, nonce="")
        with self.assertRaises(PolicyViolation): WorkflowGate({"checker"}, verifier).advance(dict(release, approval_receipt=empty_nonce), NOW)
    ...
    def test_3d_asset_gate_rejects_semantic_and_schema_drift(self) -> None:
        ...
        cases = [
            ("uv_sets", 0), ("collision", False), ("license", "unknown"),
            ("scale_meters", 0), ("poly_count", 20000), ("poly_count", 1.5),
            ("pbr_maps", ["base_color", "normal", "roughness", "metallic", "normal"]),
        ]
        for field, value in cases:
    ...
    def test_policy_rejects_missing_or_unexpected_schema_fields(self) -> None:
        ...
        with self.assertRaises(PolicyViolation): AdmissionExecutor(minimal, self.trusted_auth)
        extra = dict(self.policy, unexpected=True)
        with self.assertRaises(PolicyViolation): AdmissionExecutor(extra, self.trusted_auth)
        null_auth = dict(self.policy, auth_required=None)
        with self.assertRaises(PolicyViolation): AdmissionExecutor(null_auth, self.trusted_auth)
